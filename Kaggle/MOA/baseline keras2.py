...
valid_inds = np.random.choice(range(train_features.shape[0]), round(0.15*train_features.shape[0]), replace = False)
train_inds = np.setdiff1d(range(train_features.shape[0]), valid_inds)
train_data , train_target = train_features.to_numpy()[train_inds,:]   , train_targets.to_numpy()[train_inds]
valid_data , valid_target = train_features.to_numpy()[valid_inds,:] , train_targets.to_numpy()[valid_inds]


model = keras.Sequential()
model.add(keras.layers.BatchNormalization())
model.add(tfa.layers.WeightNormalization(keras.layers.Dense(877, activation='relu')))
model.add(keras.layers.BatchNormalization())
model.add(keras.layers.Dropout(0.7))
model.add(tfa.layers.WeightNormalization(keras.layers.Dense(877, activation='relu')))
model.add(keras.layers.BatchNormalization())
model.add(keras.layers.Dropout(0.6))
model.add(tfa.layers.WeightNormalization(keras.layers.Dense(512, activation='relu')))
model.add(keras.layers.BatchNormalization())
model.add(keras.layers.Dense(206, activation='sigmoid'))

model.compile(optimizer=keras.optimizers.SGD(learning_rate=5e-6, momentum=0.9) ,loss='binary_crossentropy')
