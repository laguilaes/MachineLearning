...
#%%
#Learning rate de 1 para los 10 primeros steps, 0.1 para los steps 10 a 25, y 0.01 en adelante
step = tf.Variable(0, trainable=False)
schedule = tf.optimizers.schedules.PiecewiseConstantDecay(
    [10, 25], [1e-0, 1e-1, 1e-2])
...
def create_model(num_columns):
    ...
    model.compile(optimizer=tfa.optimizers.AdamW(lr = 2e-3, weight_decay = 1e-5, clipvalue = 700), loss='binary_crossentropy', 
                  )
    return model
...
for seed in range(N_STARTS):
    for n, (tr, te) in enumerate(MultilabelStratifiedKFold(n_splits=7, random_state=seed**2, shuffle=True).split(train_targets, train_targets)):
        ...
        cb_checkpt = ModelCheckpoint(checkpoint_path, monitor = 'val_loss', verbose = 1, save_best_only = True, save_weights_only = True, mode = 'min')
        ...
        model.fit(train.values[tr][:, top_feats],
                  ...
                  callbacks=[reduce_lr_loss, early], verbose=2
                 )
